    ...
    if (!customOptions) {
        if (typeof options.preset === 'object') {
            presets.default = options.preset
                ...
                .map(line => line.map(k => (typeof k === 'string' ? (0, presets_js_1.findKey)(k) : [k[0], k[0], ...k.slice(1)])));
        }
    ...
    for (const lineKeys of keyboardPreset) {
        ...
        for (const [key, code, ...rest] of lineKeys) {
            ...
            if (key === 'Backspace' ||
                key === 'Enter' ||
                key === 'Shift' ||
                [presets_js_1.ALPHABET_KEY, presets_js_1.NUMERIC_KEY, presets_js_1.SYMBOL_KEY, presets_js_1.NUMPAD_KEY, presets_js_1.DOTCOM_KEY].includes(key)) {
